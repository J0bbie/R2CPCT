...
#' DESeq2.allSamples <- DESeq2::DESeq(DESeq2.allSamples, test = 'Wald', parallel = T, BPPARAM = BiocParallel::MulticoreParam(workers = 20))
...
retrieveDESeq2Results <- function(DESeq2.Object, contrast, nThreads = 20){
    ...
    # Get diff. results.
    results <- DESeq2::results(DESeq2.Object, pAdjustMethod = 'BH', filterFun = IHW::ihw, parallel = T, BPPARAM = BiocParallel::MulticoreParam(workers = nThreads), tidy = F, contrast = contrast)

    # Shrink the LFC.
    results.LFC <- DESeq2::lfcShrink(DESeq2.Object, res = results, parallel = T, BPPARAM = BiocParallel::MulticoreParam(workers = nThreads), contrast = contrast, type = 'ashr')
    ...
    # Add the SYMBOL.
    results.LFC <- results.LFC %>% dplyr::left_join(tibble::as_tibble(S4Vectors::mcols(R2CPCT::GENCODE.v35)) %>% dplyr::distinct(SYMBOL, ENSEMBL), by = 'ENSEMBL')
